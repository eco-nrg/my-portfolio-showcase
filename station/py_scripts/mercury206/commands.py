# coding=utf8
from mercury206.communications import send_command, command_shortcut
from mercury206.utils import digitize, digitized_triple
...
def turn_relay(port, address, state):
    data = send_command(port, address, 0x71, 0x00 if state else 0xAA)
    if state:
        send_command(port, address, 0x71, 0x5A)
    return data


def get_relay(port, address):
    data = send_command(port, address, 0x6D)
    print(data)
    if data[1] == 0x00:
        return True
    elif data[1] == 0xAA:
