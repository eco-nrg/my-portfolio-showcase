...
with database.atomic():
    database.execute_sql(
        f'ALTER TABLE {table_name} ALTER COLUMN "AskedIn" TYPE timestamp without time zone USING to_timestamp("AskedIn" / 1000.0)'
    )
    database.execute_sql(
        f'ALTER TABLE {table_name} ALTER COLUMN "SolvedAt" TYPE timestamp without time zone USING to_timestamp("SolvedAt" / 1000.0)'
    )
